# AWS VPC Configuration with Network Segmentation
# Creates isolated subnets for different application tiers
...
resource "aws_subnet" "private_app" {
  ...
  tags = {
    Name        = "${var.app_name}-${var.environment}-private-app-subnet-${count.index + 1}"
    Environment = var.environment
    Type        = "private"
    Tier        = "application"
  }
...
resource "aws_subnet" "private_db" {
  ...
  tags = {
    Name        = "${var.app_name}-${var.environment}-private-db-subnet-${count.index + 1}"
    Environment = var.environment
    Type        = "isolated"
    Tier        = "database"
  }
...
# Route Tables for Private App Subnets (with NAT)
resource "aws_route_table" "private_app" {
  ...
  tags = {
    Name        = "${var.app_name}-${var.environment}-private-app-rt-${count.index + 1}"
    Environment = var.environment
    Type        = "private"
  }
}
...
resource "aws_route_table" "private_db" {
  ...
  tags = {
    Name        = "${var.app_name}-${var.environment}-private-db-rt-${count.index + 1}"
    Environment = var.environment
    Type        = "isolated"
  }
}
